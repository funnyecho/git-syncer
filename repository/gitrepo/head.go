...
func (r *repo) GetHead() (string, error) {
	...
	if head == "" {
		return "", errors.NewError(errors.WithCode(exitcode.RepoHeadNotFound), errors.WithMsg("failed to get repo head"))
	}
...
func (r *repo) GetHeadSHA1() (string, error) {
	...
	if sha1 == "" {
		return "", errors.NewError(errors.WithCode(exitcode.RepoHeadNotFound), errors.WithMsg("failed to get repo head sha1"))
	}
